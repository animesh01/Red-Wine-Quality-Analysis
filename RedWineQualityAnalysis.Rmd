...
Observation:
The above bar chart visualization shows distribution for quality variable. Since we observe no break points in the distribution hence we can infer that it is uniformly distributed across all values. Also we can observe that the data is not normally distributed. Infact the distribution is left skewed.

```{r}
# We will plot a density distribution visual to observe the distribution after log transformation
library("ggplot2")
quality_viz <-
  ggplot(aes(x = log(quality), fill = I('pink')), data = WineQ) + 
  geom_density()
quality_viz
```
...
Summarizing the quality variable after transformation

```{r}
# Checking summary after the log transformation
summary(log(WineQ$quality))
```
...
library(grid)
corPlot <-
ggplot(aes(x = log(alcohol), y = log(quality)), data = WineQ) +
geom_point() +
...
a <- anova(lm(log(alcohol) ~ log(quality), data = WineQ))
a
```
Observation:
...
library(grid)
corPlot <-
ggplot(aes(x = log(sulphates), y = log(quality)), data = WineQ) + geom_point() +
stat_smooth(method = 'lm')
...
```{r}
# Summary statistics after anova analysis
anova_sul_qual <- anova(lm(log(sulphates) ~ log(quality), data = WineQ))
anova_sul_qual
```
Observation:
...
library(grid)
corPlot <-
ggplot(aes(x = log(citric.acid), y = log(quality)), data = WineQ) + geom_point() +
stat_smooth(method = 'lm')
...
```{r}
# Summary statistics after anova analysis
anova_citric_quality <- anova(lm(citric.acid ~ log(quality), data = WineQ))
...
ggplot(aes(x = sulphates, y = citric.acid), data = WineQ) +
  geom_point(aes(color = quality, alpha = 0.0005)) +
